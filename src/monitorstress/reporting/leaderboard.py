...
* ``clean_pass_rate`` — fraction of trajectories with
  :attr:`~monitorstress.core.verdict.IntegrityLabel.CLEAN`.
* ``non_exploit_pass_rate`` — pass rate after stripping only
  trajectories labeled
  :attr:`~monitorstress.core.verdict.IntegrityLabel.EXPLOIT`.
...
class ComparisonReport(BaseModel):
    """Output of ``monitorstress compare`` — two submissions side-by-side."""
