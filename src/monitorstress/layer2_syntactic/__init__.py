...
* :mod:`monitorstress.layer2_syntactic.code_extractor` — pull every
  ``before`` / ``after`` source pair out of a trajectory.
* :mod:`monitorstress.layer2_syntactic.exploit_signatures` — the curated
  catalog of exploit signatures (one per entry in the taxonomy).
* :mod:`monitorstress.layer2_syntactic.ast_scanner` — the scanner that
