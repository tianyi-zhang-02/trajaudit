...
:class:`~monitorstress.layer2_syntactic.code_extractor.CodeEdit` instances
and the catalog from
:func:`~monitorstress.layer2_syntactic.exploit_signatures.get_all_signatures`,
it parses each edit's ``after_source`` and returns one
:class:`~monitorstress.core.verdict.SyntacticFinding` per matched
signature.
"""

from __future__ import annotations

from monitorstress.core.verdict import SyntacticFinding
from monitorstress.layer2_syntactic.code_extractor import CodeEdit
from monitorstress.layer2_syntactic.exploit_signatures import (
    ExploitSignature,
    get_all_signatures,
)
